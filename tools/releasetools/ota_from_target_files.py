...
def WriteFullOTAPackage(input_zip, output_zip):
  ...
  assert block_based is False
  print("Block based:" + str(block_based))
...
""" % bcb_dev)
  ...
  if(block_based):
	print("Error, block_based is true")
	assert block_based is False
  else:
	common.ZipWriteStr(output_zip, "system/build.prop",
                     ""+input_zip.read("SYSTEM/build.prop"))
...
""" % bcb_dev)
  ...
  if updating_boot:
    ...
    if d is None:
      ...
      print ("boot      target: %d  source: %d  diff: %d" % (target_boot.size, source_boot.size, len(d)))
